...
export class CustomFieldsController extends AppController<
	CustomFieldDocument,
...
> {
	...
	@ApiResponse({
		status: 200,
		description:
			"Return all custom fields for the current owner and schema.",
	})
	@ApiResponse({ status: 404, description: "Custom fields not found." })
	@UseGuards(AuthGuard)
	@Get("me/:schema")
	async findAllOwnerCustomsFields(
		...
		try {
			...
			if (error.message === "Not Found") {
				...
				console.error(
					"CustomFieldsController > findAllOwnerCustomsFields : ",
					error,
				);
				return this.responsesHelper.getResponse({
					res,
	...
	@ApiResponse({
		status: 200,
		description:
			"Return the custom field for the current owner and schema.",
	})
	@ApiResponse({ status: 404, description: "Custom field not found." })
	@Get(":id/me/:schema")
	async findOneOwnerCustomsFields(
		@Res() res: Response,
		@Req() req: Request,
		@Param("id") id: string,
		@Param("schema") schema: string,
		...
		try {
			...
			if (error.message === "Not Found") {
				...
				console.error(
					"CustomFieldsController > findOneOwnerCustomsFields : ",
					error,
				);
				return this.responsesHelper.getResponse({
					res,
	...
	@ApiOperation({
		summary:
			"Update positions of custom fields for the current owner and schema",
	})
	@ApiResponse({
		status: 200,
		description:
			"The positions of the custom fields have been successfully updated.",
	})
	@ApiResponse({ status: 500, description: "Internal server error." })
	@UseGuards(AuthGuard)
	@Put("me/:schema")
	async updatePositions(
		@Body() updateCustomFieldDtos: UpdateCustomFieldDto[],
		@Res() res: Response,
		@Req() req: Request,
		@Param("schema") schema: string,
	) {
		...
		try {
			const result = await this.customFieldsService.updatePositions(
				updateCustomFieldDtos,
			);
			const data = await this.customFieldsService.findWhere({
				where: {
					ownerId: customer.id.toString(),
