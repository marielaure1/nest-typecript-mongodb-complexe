...
describe("CustomFieldsController", () => {
	...
	const mockCustomFieldsService = {
		create: jest
			.fn()
			.mockImplementation((dto: CreateCustomFieldDto) =>
		...
		findOne: jest
			...
			.mockImplementation((id: string) =>
				Promise.resolve({ _id: id, ...mockCustomField }),
			),
		update: jest
			.fn()
			.mockImplementation((id: string, dto: UpdateCustomFieldDto) =>
	...
	it("should handle internal server errors when retrieving custom fields for the current owner and schema", async () => {
		jest.spyOn(service, "findWhere").mockRejectedValueOnce(
			new Error("Internal server error"),
		);
	...
	it("should update the positions of custom fields", async () => {
		...
		jest.spyOn(service, "updatePositions").mockResolvedValueOnce(
			updateDtos,
		);
	...
	it("should handle internal server errors when updating positions of custom fields", async () => {
		...
		jest.spyOn(service, "updatePositions").mockRejectedValueOnce(
			new Error("Internal server error"),
		);
