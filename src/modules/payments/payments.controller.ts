...
@ApiTags("payments")
@Controller("payments")
export class PaymentsController extends AppController<
	PaymentDocument,
	CreatePaymentDto,
	CreatePaymentDto
> {
	...
	async createCheckoutSession(
		...
		const createEphemeralKey =
			await this.paymentsStripeService.createEphemeralKey(
				test.customerId,
			);
		const paymentIntent =
			await this.paymentsStripeService.createPaymentIntent({
				amount: test.amount,
