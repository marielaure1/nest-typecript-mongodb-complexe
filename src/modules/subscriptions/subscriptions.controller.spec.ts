...
describe("SubscriptionsController", () => {
	...
	const mockStripeCustomersService = {
		updateCustomer: jest
			.fn()
			.mockResolvedValue({ id: "stripe_customer_1" }),
		deleteCustomer: jest
			.fn()
			.mockResolvedValue({ id: "stripe_customer_1" }),
	};
	...
	const mockStripeSubscriptionsService = {
		createSubscription: jest
			...
			.mockImplementation((dto: any) =>
				Promise.resolve({ id: "stripe_subscription_1", ...dto }),
			),
		updateSubscription: jest
			.fn()
			.mockImplementation((id: string, dto: any) =>
				Promise.resolve({ id: "stripe_subscription_1", ...dto }),
			),
		cancelSubscription: jest
			.fn()
			.mockResolvedValue({ id: "stripe_subscription_1" }),
	};
	...
	beforeEach(async () => {
		...
		stripeCustomersService = module.get<CustomersStripeService>(
			CustomersStripeService,
		);
		stripeSubscriptionsService = module.get<SubscriptionsStripeService>(
	...
	it("Create [201] - should create a subscription", async () => {
		...
		expect(
			stripeSubscriptionsService.createSubscription,
	...
	it("Create [422] - should handle validation errors when creating a subscription", async () => {
		...
		jest.spyOn(subscriptionsService, "create").mockRejectedValueOnce([
			validationError,
		]);
	...
	it("FindAll [500] - should handle internal server errors when returning all subscriptions", async () => {
		jest.spyOn(subscriptionsService, "findAll").mockRejectedValueOnce(
			new Error("Internal server error"),
		);
	...
	it("FindOne [500] - should handle internal server errors when returning a single subscription by id", async () => {
		jest.spyOn(subscriptionsService, "findOne").mockRejectedValueOnce(
			new Error("Internal server error"),
		);
	...
	it("Update [500] - should handle internal server errors when updating a subscription by id", async () => {
		...
		jest.spyOn(subscriptionsService, "update").mockRejectedValueOnce(
			new Error("Internal server error"),
		);
	...
	it("Delete [500] - should handle internal server errors when deleting a subscription by id", async () => {
		jest.spyOn(subscriptionsService, "remove").mockRejectedValueOnce(
			new Error("Internal server error"),
		);
	...
	it("FindMySubscription [200] - should return the subscription of the current user", async () => {
		...
		expect(subscriptionsService.findWhere).toHaveBeenCalledWith({
			where: { customer: "customer_1" },
		});
	});
	...
	it("FindMySubscription [500] - should handle internal server errors when retrieving subscriptions for the current user", async () => {
		jest.spyOn(subscriptionsService, "findWhere").mockRejectedValueOnce(
			new Error("Internal server error"),
		);
	...
	it("CancelSubscription [200] - should cancel a subscription by id", async () => {
		...
		expect(
			stripeSubscriptionsService.cancelSubscription,
		).toHaveBeenCalledWith("stripe_subscription_1");
	});
