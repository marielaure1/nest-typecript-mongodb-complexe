...
export class SubscriptionsController {
	...
	constructor(
		...
		@Inject(CustomersStripeService)
		private readonly stripeCustomersService: CustomersStripeService,
		@Inject(CustomersService)
		private readonly customersService: CustomersService,
		@Inject(UsersService) private readonly usersService: UsersService,
		@Inject(PlansService) private readonly plansService: PlansService,
		@Inject(SubscriptionsStripeService)
		private readonly stripeSubscriptionsService: SubscriptionsStripeService,
	) {
	...
	async create(
		...
		try {
			const getCustomer = await this.customersService.findOne(
				createSubscriptionDto.customer.toString(),
			);
			const isFindUsers = await this.usersService.findOne(
				getCustomer.user._id?.toString(),
			);
			const isFindPlan = await this.plansService.findOne(
				createSubscriptionDto.plan.toString(),
			);
	...
	async update(
		...
		try {
			...
			if (updateSubscriptionDto.customer) {
				await this.stripeCustomersService.updateCustomer(
					subscription.stripeCustomerId,
					updateSubscriptionDto.customer,
				);
			}

			const updateSubscription = await this.subscriptionsService.update(
				id,
	...
	async delete(@Param("id") id: string, @Res() res: Response) {
		try {
			...
			await this.stripeSubscriptionsService.cancelSubscription(
				subscription.stripeSubscriptionId,
			);
			await this.stripeCustomersService.deleteCustomer(
				subscription.stripeCustomerId,
			);
	...
	async findMySubscription(@Res() res: Response, @Req() req: Request) {
		...
		try {
			const data = await this.subscriptionsService.findWhere({
				where: { customer: customer.id.toString() },
			});
			...
			const stripeSubscription =
				await this.stripeSubscriptionsService.findSubscription(
